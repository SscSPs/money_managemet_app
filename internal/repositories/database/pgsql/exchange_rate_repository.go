...
func (r *PgxExchangeRateRepository) WithTx(tx pgx.Tx) portsrepo.ExchangeRateRepositoryWithTx {
	repo := NewPgxExchangeRateRepository(r.Pool)
	repo.SetTx(tx)
	return repo
}

// BeginTx starts a new transaction.
func (r *PgxExchangeRateRepository) BeginTx(ctx context.Context) (pgx.Tx, error) {
	if tx := r.GetTx(); tx != nil {
		return tx, nil // Return existing transaction if already in one
	}
...
func (r *PgxExchangeRateRepository) ListExchangeRates(
	...
	rows, err := r.DB().Query(ctx, "SELECT * "+baseQuery, args...)
	if err != nil {
