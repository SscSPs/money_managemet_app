...
// Commit commits the provided transaction and clears internal state if it matches current tx
...
func (r *BaseRepository) WithTx(ctx context.Context, fn func(tx pgx.Tx) error) error {
	// Begin will reuse existing tx if present, else start a new one
	preExisting := r.tx != nil
	tx, err := r.Begin(ctx)
	if err != nil {
		return err
	}
	...
	if preExisting {
		return nil // outer scope will manage commit
	}
